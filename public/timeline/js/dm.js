...
function goBackToDMList() {
	...
	// Switch to DM list page
	switchPage("direct-messages", { path: "/dm" });

	// Reload conversations to ensure fresh state
	loadConversations();
}
...
function renderParticipantsList() {
	...
	participantsList.innerHTML = allParticipants
        ...
        <div class="participant-item">
          <img src="${participant.avatar || "/public/shared/default-avatar.png"}" alt="${participant.name || participant.username}" />
          <div class="participant-info">
            <span class="participant-name">${participant.name || participant.username}</span>
...
function renderAddParticipantSuggestions(users) {
	...
	suggestionsElement.innerHTML = availableUsers
		.map(
			(user) => `
      <div class="suggestion-item" onclick="addParticipantUser('${user.username}', '${user.name || ""}', '${user.avatar || ""}', '${user.id}')">
...
function renderSelectedParticipants() {
	...
	element.innerHTML = selectedParticipants
      ...
      <div class="selected-user">
        ...
        <button class="remove-user" onclick="removeParticipantUser('${user.username}')">&times;</button>
      </div>
    `,
		)
		.join("");
...
async function confirmAddParticipant() {
	...
	try {
		...
		// Add new participants to current conversation
...
function addUser(username, name, avatar) {
	...
	// Auto-suggest group chat for multiple users
	const groupToggle = document.getElementById("groupChatToggle");
...
// Handle browser back button for DM routes
window.addEventListener("popstate", (event) => {
	const currentPath = window.location.pathname;

	// If we're navigating to a DM route via back button, make sure to handle it properly
	if (currentPath === "/dm") {
		// Clear conversation state when going back to DM list
		currentConversation = null;
		currentMessages = [];

		// Only reload conversations if we're actually on the DM page
